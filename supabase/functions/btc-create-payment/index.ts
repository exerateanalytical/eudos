...
// Supports Exodus, Electrum, and other standard BIP32-based wallets
function deriveAddressFromXpub(xpub: string, index: number, derivationPath: string): string {
  try {
    ...
    let scriptType = 'p2pkh'; // Default for BIP32/BIP44 (1... addresses)
    ...
    console.log(`✅ Derived ${scriptType.toUpperCase()} address: ${address} at index ${index}`);
    return address;
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔑 Using xpub wallet: ${walletData.name}`);
    ...
    console.log(`🔨 Deriving Bitcoin address from xpub...`);
